...
const BiblicalTheologyLessons = () => {
  const apiUrl = process.env.REACT_APP_API_URL; // ${apiUrl} 
  const [biblicalTheologyDoc, setBiblicalTheologyDoc] = useState([]);
  const [pdfDoc, setPdfDoc] = useState({});
  const [activeDocument, setActiveDocument] = useState("");

  useEffect(() => {
    const fetchBiblicalTheologyDoc = async () => {
      try {
        const response = await axios.get(`${apiUrl}biblical-theology-lessons`);
        setBiblicalTheologyDoc(response.data);
      } catch (error) {
        console.error("Error fetching biblical theology lessons:", error);
      }
    };

    fetchBiblicalTheologyDoc();
  }, []);
      ...
      const fetchPdfDoc = async () => {
        try {
          const response = await axios.get(
            `${apiUrl}biblical-theology-lessons/${activeDocument}`
          );
  ...
  return (
    <>
      <HelmetProvider>
        <Helmet>
          <meta charSet="utf-8" />
          <title>Biblical Theology Lessons | Baptists for Africa</title>
        </Helmet>
        <>
          <Navbar2 />
          <PageHead name="Biblical Theology Lessons" />
              ...
              <section className="sticky col-span-3 h-full w-full border-r lg:bg-white">
                <ul className="hidden lg:block lg:w-full">
                  <li className=" hidden lg:block lg:p-2 lg:w-full lg:border-b lg:font-extrabold">
                    BIBLICAL THEOLOGY DOCUMENT LESSONS:
                  </li>
                  {biblicalTheologyDoc.map((doc) => (
                    <li
                      key={doc.slug}
                ...
                <div className="lg:hidden mx-4 mt-4 p-4 flex flex-col gap-4 justify-center items-center bg-white rounded-md shadow-xl">
                  ...
                  <div className="flex rounded-lg">
                    ...
                    >
                      {biblicalTheologyDoc.map((doc, index) => (
                        <option
                          value={doc.slug}
...
export default BiblicalTheologyLessons;
