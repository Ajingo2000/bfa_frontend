...
const DisciplerClassLessons = () => {
  const [disciplerClassDoc, setDisciplerClassDoc] = useState([]);
  ...
  useEffect(() => {
    const fetchDisciplerClassDoc = async () => {
      try {
        const response = await axios.get(`${apiUrl}discipler-class-lessons`);
        setDisciplerClassDoc(response.data);
      } catch (error) {
        console.error("Error fetching discipler class documents:", error);
      }
    };

    fetchDisciplerClassDoc();
  }, []);
      ...
      const fetchPdfDoc = async () => {
        try {
          const response = await axios.get(
            `${apiUrl}discipler-class-lessons/${activeDocument}`
          );
  ...
  return (
    <>
      <HelmetProvider>
        <Helmet>
          <meta charSet="utf-8" />
          <title>Discipler Class Lessons | Baptists for Africa</title>
        </Helmet>
        <>
          <Navbar2 />
          <PageHead name="Discipler Class Lessons" />
              ...
              <section className="sticky col-span-3 h-full w-full border-r lg:bg-white">
                <ul className="hidden lg:block lg:w-full">
                  <li className=" hidden lg:block lg:p-2 lg:w-full lg:border-b lg:font-extrabold">
                    DISCIPLER CLASS DOCUMENT LESSONS:
                  </li>
                  {disciplerClassDoc.map((doc) => (
                    <li 
                      key={doc.slug}
                ...
                <div className="lg:hidden mx-4 mt-4 p-4 flex flex-col gap-4 justify-center items-center bg-white rounded-md shadow-xl">
                  ...
                  <div className="flex rounded-lg">
                    ...
                    >
                      {disciplerClassDoc.map((doc, index) => (
                        <option
                          value={doc.slug}
...
export default DisciplerClassLessons;
