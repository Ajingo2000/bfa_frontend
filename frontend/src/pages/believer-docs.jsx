...
const NewBelieverDocs = () => {
  ...
  useEffect(() => {
    const fetchBelieverDoc = async () => {
      try {
        const response = await axios.get(`${apiUrl}new-believer-documents`);
        setBelieverDoc(response.data);
      } catch (error) {
      ...
      const fetchPdfDoc = async () => {
        try {
          const response = await axios.get(
            `${apiUrl}new-believer-documents/${activeDocument}`
          );
        ...
        <section className="bg-gray-100">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-12 w-full h-full">
            <section className="sticky col-span-3 h-full w-full border-r lg:bg-white ">
              ...
              <div className="lg:hidden mx-2 mt-4 p-4 flex flex-col gap-4 justify-center items-center bg-white rounded-md shadow-xl">
...
export default NewBelieverDocs;
